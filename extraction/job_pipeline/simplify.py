...
import requests
from .constants import ADVANCED_DEGREE_KEYWORDS, BLOCKED_COMPANIES, CITIZENSHIP_KEYWORDS, DEFAULT_RAW_SIMPLIFY, KEEP_CATEGORY_KEYWORDS, SIMPLIFY_GITHUB_URL, SIMPLIFY_LISTINGS_URL
...
def simplify_listing_passes_filters(listing: dict[str, Any]) -> tuple[bool, str]:
    ...
    scan = " ".join(clean_space(listing.get(k)) for k in ("title", "category", "description", "requirements"))
    if _contains_any(scan, CITIZENSHIP_KEYWORDS):
        return False, "citizenship_keyword"
    if not _contains_any(f"{listing.get('category','')} {listing.get('title','')}", KEEP_CATEGORY_KEYWORDS):
...
def simplify_to_master_record(listing: dict[str, Any]) -> dict[str, Any]:
    ...
    record.update({
        ...
        "seniority": "new_grad",
        "remote_policy": "remote" if "remote" in " ".join(locations).lower() else "",
        "job_url": job_url,
        "visa_sponsorship_policy": clean_space(listing.get("sponsorship")),
        "keywords": [clean_space(listing.get("category"))] if listing.get("category") else [],
        "confidence": "medium",
...
def build_simplify_records(raw_output_path: str = DEFAULT_RAW_SIMPLIFY) -> list[dict[str, Any]]:
    out = []
